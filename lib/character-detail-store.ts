...
async function readStoredCharacterDetail(
  realmSlug: string,
  name: string,
  ...
  try {
    ...
    // A Redis hiccup must never crash the page — fall through to the live
    // fallback (or minimal header) instead.
    console.error("[char-detail] read failed:", e);
    return null;
  }
}

/**
 * Read a character's precomputed detail. null when missing, version-stale, on a
 * Redis error, or when Redis isn't configured — the caller then live-fetches.
 *
 * Wrapped in unstable_cache: the character page is statically generated (ISR),
 * and @upstash/redis reads use an uncached fetch under the hood — calling that
 * directly during static generation throws DYNAMIC_SERVER_USAGE (a 500). The
 * cache makes the read ISR-compatible (treated as cacheable data, refreshed on
 * the page's hourly revalidation) and the result is keyed per (realm, name).
 */
export const getStoredCharacterDetail = unstable_cache(
  readStoredCharacterDetail,
  ["character-detail-store-v1"],
  { revalidate: 3600 },
);
